...
// icon notifikasi
import { SparklesIcon, ExclamationTriangleIcon, ChevronDownIcon } from '@heroicons/vue/24/outline'
...
const stepProgress = computed(() => Math.round((currentStep.value / steps.length) * 100))
...
const validateCurrentStep = () => {
  ...
  if (currentStep.value === 1) {
    ...
    if (!form.program_studi_1) return setError('Pilih Program Studi 1 terlebih dahulu.')
    ...
    if (nik.length !== 16 || !isDigits(nik)) return setError('NIK harus 16 digit angka.')
    if (kk.length !== 16 || !isDigits(kk)) return setError('Nomor KK harus 16 digit angka.')
  ...
  if (currentStep.value === 3) {
    if (
      ...
      return setError('Lengkapi email, nomor WhatsApp, kata sandi, dan konfirmasi kata sandi akun PMB.')
    }
    if (form.kata_sandi !== form.konfirmasi_kata_sandi) {
      return setError('Kata sandi dan konfirmasi kata sandi tidak sama.')
    }
    if (String(form.kata_sandi).length < 8) return setError('Kata sandi minimal 8 karakter.')
    return true
  }

  // STEP 4
  if (currentStep.value === 4) {
    if (!(form.nomor_hp || '').trim()) return setError('Nomor WhatsApp wajib diisi pada langkah Akun & WhatsApp.')
    if (!form.otp_terverifikasi) return setError('Mohon kirim dan verifikasi kode OTP WhatsApp terlebih dahulu.')
    return true
  }

  // STEP 5 – wajib KTP + KK
  if (currentStep.value === 5) {
    if (!form.kip_ktp_file || !form.kip_kk_file) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.foto_file) fd.append('foto', form.foto_file)
    ...
    if (!response.ok) {
      let errorText = 'Terjadi kesalahan saat mengirim pendaftaran KIP.'
      try {
    ...
    submitMessage.value = data.message || 'Pendaftaran KIP berhasil disimpan ke sistem.'
    console.log('Data pendaftaran KIP:', data.data)
  } catch (error) {
    console.error(error)
    submitType.value = 'error'
...
.dark ::placeholder { color: #ffffff; opacity: 0.9; }

input, textarea, select { color: #0f172a; }
.dark input, .dark textarea, .dark select { color: #ffffff; }

select option { color: inherit; }
.dark select option { color: #ffffff; background-color: #020617; }

.dark input[type='date'] { color-scheme: dark; }
.dark input[type='date']::-webkit-calendar-picker-indicator { filter: invert(1) brightness(1.2); opacity: 0.9; }
...
.dark input[type='date']::-webkit-datetime-edit-year-field { color: #ffffff; }
          ...
          <div class="max-w-4xl mx-auto">
            <header class="mb-4 md:mb-5">
              <h1 class="text-xl md:text-2xl font-semibold text-slate-900 dark:text-slate-50">
                Pendaftaran Jalur KIP
              </h1>
              <p class="text-sm text-slate-600 dark:text-slate-300">
                Isi formulir KIP secara bertahap hingga selesai, lalu kirim pendaftaran.
              </p>
            </header>
